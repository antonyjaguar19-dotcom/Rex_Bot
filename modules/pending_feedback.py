...
def add(script: dict, owner_id: int, channel_id: int, reason: str = "paused") -> None:
    ...
    sid = script.get("script_id") or script.get("_id")
    if not sid:
        raise ValueError("Cannot store pending feedback: script has no script_id or _id.")
    data = _load_all()
    data[sid] = {
        "script": script,
        "owner_id": owner_id,
    ...
    _save_all(data)
    log.info(f"Pending feedback stored for {sid} (reason={reason}).")

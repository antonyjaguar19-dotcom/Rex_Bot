...
def assemble_audio_first(script_id: str, progress_cb: Optional[Callable] = None) -> dict:
    """Stitch silent per-shot clips + the single master narration into finals.

    Reads `_master_audio` from the script JSON. Clips are hard-cut back-to-back
    (each is already exactly its segment-window length), then the master VO is
    laid over the whole timeline so every cut lands in a natural pause.
    """
    import json
    _asm_start = time.time()
    if not FFMPEG_EXE.exists():
    ...
    spath = SCRIPTS_DIR / f"script_{script_id}.json"
    if not spath.exists():
        raise FileNotFoundError(f"No script JSON: {spath}")
    script = json.loads(spath.read_text(encoding="utf-8"))
    master_audio = Path(script.get("_master_audio", ""))
    if not master_audio.exists():
        raise FileNotFoundError(
            f"Master narration audio missing: {master_audio}. "
            f"Was this script generated by the audio-first pipeline?"
        )

    clips = _find_clips_for_script(script_id)      # latest rev per shot, ordered
    if not clips:
        raise RuntimeError(f"No clips to assemble for {script_id}")

    total_dur = _probe_duration(master_audio)
    ...
    # Background music under the narration (best-effort, reuse existing path).
    try:
        from modules import music_generator as mg
        mood = mg.get_effective_mood()
        if progress_cb:
            progress_cb(f"generating background music ({mood})...")
        music_track = mg.generate_music(mood=mood, duration_sec=total_dur,
                                        script_id=script_id, progress_cb=progress_cb)
        if music_track and music_track.exists():
            for aspect_key in ("9x16", "16x9", "1x1"):
                silent_final = outputs[aspect_key]
                tmp_mixed = silent_final.with_suffix(".mixed.mp4")
                try:
                    _mix_music_under_narration(silent_final, music_track, tmp_mixed, -12.0)
                    silent_final.unlink(missing_ok=True)
                    tmp_mixed.rename(silent_final)
                except Exception as e:
                    log.exception(f"Music mix failed for {aspect_key}: {e}")

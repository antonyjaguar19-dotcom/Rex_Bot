...
async def generate_and_post_storyboard(
    ...
    for frame in result.frames:
        ...
        embed = discord.Embed(
            title=f"Shot {frame.shot_number}",
            description=f"🎙️ *{narration}*",
    ...
    approval_msg = await channel.send(
        ...
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"✅ Approve → send to Phase 5 (video generation, coming soon)\n"
        f"❌ Reject → tell me what to change (e.g., \"redo shot 2\")\n"
        f"⏹️ Stop this storyboard entirely\n"
        f"💾 Pause (resume later)"
    )
...
async def regenerate_shots(
    ...
    for frame in all_new_frames:
        ...
        if not abs_path.exists():
            continue
        embed = discord.Embed(
            title=f"Shot {frame.shot_number} (revised)",
            description=f"🎙️ *{narration}*",
    ...
    approval_msg = await channel.send(
        ...
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"✅ Approve · ❌ Reject again · ⏹️ Stop · 💾 Pause"
    )
    await approval_msg.add_reaction("✅")
    await approval_msg.add_reaction("❌")
    await approval_msg.add_reaction("⏹️")
...
def extract_shot_numbers(notes: str) -> list[int]:
    """Parse revision notes looking for shot references."""
    nums = set()
    for m in re.finditer(r"shot\s*#?\s*(\d)", notes.lower()):
        n = int(m.group(1))
        if 1 <= n <= 4:
            nums.add(n)
    for m in re.finditer(r"scene\s*#?\s*(\d)", notes.lower()):
        n = int(m.group(1))
        if 1 <= n <= 4:
            nums.add(n)
    word_map = {
    ...
    for word, n in word_map.items():
        if re.search(rf"\b{word}\s+(shot|scene|frame)", notes.lower()):
            nums.add(n)
    return sorted(nums)

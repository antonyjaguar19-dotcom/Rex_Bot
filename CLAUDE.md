...
- **Voice clips are normalised to mono 16k WAV on ingest** (`mascot_library._to_wav_mono16k`). Discord always did; the dashboard stored the upload raw, so a mascot cloned from a **48 kHz mp3**. **CONFIRMED by re-voicing the same lines from the same clip, converted:** hook 0.12s → **4.08s**, outro 0.60s → **3.08s**, fact 4.76s → 4.80s. The mp3 reference WAS the trigger — never let a raw upload become the clone reference.

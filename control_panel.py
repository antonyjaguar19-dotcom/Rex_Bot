...
def _home_embed() -> discord.Embed:
    ...
    e.add_field(name="🎨 Storyboards",  value="Frames · regen · continuity", inline=True)
...
class StoryboardsView(_SubView):
    ...
    @ui.button(label="🧬 Continuity", style=discord.ButtonStyle.secondary, row=1, custom_id="cp:stb:cc")
    async def b4(self, i, b): await i.response.send_modal(ScriptIdModal("Continuity Check", "continuity"))
...
class _VideoModelSelect(ui.Select):
    def __init__(self):
        from modules import model_registry
        try:
            current = model_registry.get_active("video_backend")
            available = model_registry.list_available("video_backend")
            current_id = current["_id"] if current else None
        except Exception:
            available, current_id = {}, None
        options = [
        ...
        super().__init__(
            placeholder="🎥  Video backend…",
            options=options, min_values=1, max_values=1,
            custom_id="cp:mdl:sel:video",
        )

    async def callback(self, interaction):
        choice = self.values[0]
        await _run_cmd(interaction, "switch_model", backend_id=choice, backend_type="video")


class ModelsView(_SubView):
    def __init__(self):
        super().__init__()
        # Insert select menus before the Home button (row 4)
        self.add_item(_ImageModelSelect())
        self.add_item(_VideoModelSelect())

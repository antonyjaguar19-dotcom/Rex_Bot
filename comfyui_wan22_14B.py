...
WORKFLOW_PATH = PROJECT_ROOT / "05_Config" / "workflows" / "wan22_14B_i2v_api.json"
...
class Backend(VideoBackend):
    ...
    def __init__(self, config: dict):
        ...
        log.info(
            ...
            f"4-step LoRA: {self.enable_4steps_lora}"
        )
    ...
    def generate(
        ...
        log.info(
            ...
            f"(~{int(duration*fps+1)} frames), 4step_lora={self.enable_4steps_lora}, "
        ...
        # 4. Wait — generous timeout (full path can be 15+ min)
        timeout = 600 if self.enable_4steps_lora else 1800
    ...
    def _build_workflow(self, prompt: str, negative_prompt: str,
        ...
        for node_id, node in wf.items():
            ...
            # --- 4-step LoRA toggle ---
                ...
                inputs["value"] = self.enable_4steps_lora
            ...
            # --- Title-targeted primitives ---
                ...
                # CFG primitives feed into the switch — there are TWO (one per LoRA mode).
                # Inject the same effective CFG into both, the switch picks the active one.

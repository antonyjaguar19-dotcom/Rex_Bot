...
def _build_workflow(input_video_relative_path: str) -> dict:
    """Load template, inject the input video path."""
...
def upscale_clip(video_path: Path, progress_cb: Optional[Callable] = None) -> Path:
    ...
    # Check audio presence BEFORE we touch the file
    had_audio = _has_audio_stream(video_path)
    log.info(f"Source audio: {'present' if had_audio else 'none'} — {video_path.name}")
    ...
    try:
        ...
        workflow = _build_workflow(staged_name)
